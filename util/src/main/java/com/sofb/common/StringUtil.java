...
public class StringUtil {
     ...
     * <code>toString</code>.
     *
     * @param localeString the locale string, following <code>java.util.Locale</code>'s toString format ("en", "en_UK", etc).
     ...
     * <p>
     * The given delimiters string is supposed to consist of any account of delimiter characters. Each of those
    ...
    public static Set<Long> commaDelimitedListToLongSet(String str) {
        ...
        for (String token : tokens) {
            if (NumberUtil.isNumber(token)) {
                set.add(NumberUtil.toLong(token));
            }
        }
        return set;
    }
    ...
    public static boolean isDefinedPattern(String str, String pattern) {
        Assert.notNull(str, "str字符串不能为空");
        Assert.notNull(pattern, "pattern正则表达式不能为空");
     ...
     *
     * @param str1    将被替换的源字符串
     * @param pattern 指定的将被替换的字符串符合的模式（pattern）
     * @param str2    用来替换掉符合模式字符串的字符串
     */
     ...
     *
     * @param input            本分割的字符串或者StringBuffer对象
     * @param delimiterPattern 分割字符
     * @param limit            参数控制模式应用的次数，因此影响结果数组的长度。如果该限制 n 大于 0，则模式将被最多应用 n - 1 次，数组的长度将不会大于 n，而且数组的最后项将包含超出最后匹配的定界符的所有输入。如果 n
